...
module.exports = function (io) {
  ...
  // GET /api/messages/:leadId - get messages for a lead ( lấy tin nhắn từ khách hàng)
  ...
  // POST /api/messages - send a message (supports optional file upload) (gửi tin nhắn từ nhân viên)
  router.post('/', upload.single('file'), async (req, res) => {
    try {
      ...
      // Optional: verify lead exists ( kiểm tra khách hàng có tồn tại không)
      const lead = await Lead.findById(leadId);
      if (!lead) {
        return res
          .status(404)
